...
def login():
    if request.method == "POST":
        ...
        user = function.login(mydb, username, password, checkadmin)
        ...
        if user:
            ...
            if int(admin) == 4512:
                ...
                session["checkadmin"] = "0"
            session["account_number"] = user[6]
            session["balance"] = float(user[7])
    ...
    if request.method == "POST":
        ...
        if password == confirm_password:
            try:
                if len(pin) == 4:
                    mydb = function.connect_to_database()
                    for l in reversed(email):
                        domain += l
                    if domain[0:7] == "oc.knab":
                        admin = True
                    create = function.create_account(
                        mydb, username, password, email, int(pin), admin
                    )
                    function.close_connection(mydb)
                    flash("Account created successfully")
                    return redirect(url_for("login"))
                else:
                    flash("Invalid PIN# please make sure it is 4 digits")
