...
def login(mydb, username, password, is_admin):
    try:
        # Check if account exists
        cursor = mydb.cursor()
        cursor.execute(
            "SELECT * FROM FastTwitch.user WHERE username = %s AND password = %s AND is_admin = %s",
...
def check_balance(mydb, account_number, user_id=None):
    try:
        ...
        cursor = mydb.cursor()
        cursor.execute(
            "SELECT balance FROM FastTwitch.user WHERE account_number = %s OR user_id = %s",
...
def check_pin(mydb, account_number, pin):
    try:
        ...
        cursor = mydb.cursor()
        cursor.execute(
            "SELECT pin FROM FastTwitch.user WHERE account_number = %s",
            (account_number,),
        )
...
def deposit_funds(mydb, account_number, amount):
    try:
        ...
        cursor = mydb.cursor()
        cursor.execute(
            "UPDATE FastTwitch.user SET balance = balance + %s WHERE account_number = %s",
...
def create_account(
    ...
    try:
        ...
        cursor = mydb.cursor()
        cursor.execute(
            "INSERT INTO FastTwitch.user (user_id,username,email,password,pin,is_admin,account_number,balance) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
            (user_id, username, email, password, pin, is_admin, account_number, 5.0),
        )
        mydb.commit()
        cursor.close()
...
def reset_auto_increment(mydb):  #
    try:
        ...
        cursor = mydb.cursor()
        cursor.execute("ALTER TABLE FastTwitch.user AUTO_INCREMENT = 1")
        mydb.commit()
        cursor.close()
...
def get_user_data(mydb, account_number):
    cursor = mydb.cursor()
    cursor.execute(
        "SELECT * FROM FastTwitch.user WHERE account_number = %s", (account_number,)
    )
...
def view_database(mydb):
    try:
        ...
        cursor = mydb.cursor()
        cursor.execute("SELECT * FROM FastTwitch.user")
